...
//
// Combined variant: normals are computed analytically from the SDF voxel field
// via trilinear gradient interpolation — no separate normal generation pass needed.
...
// ---- Analytical normal computation from SDF ----

int voxelIdx(int x, int y, int z)
{
...
vec3 sdfGrad(ivec3 p)
{
	float dx = _Voxels[voxelIdx(p.x+1,p.y,p.z)] - _Voxels[voxelIdx(p.x-1,p.y,p.z)];
	float dy = _Voxels[voxelIdx(p.x,p.y+1,p.z)] - _Voxels[voxelIdx(p.x,p.y-1,p.z)];
	float dz = _Voxels[voxelIdx(p.x,p.y,p.z+1)] - _Voxels[voxelIdx(p.x,p.y,p.z-1)];
	float len = length(vec3(dx, dy, dz));
	return (len > 1e-6) ? vec3(dx, dy, dz) / len : vec3(0.0, 1.0, 0.0);
}
...
vec3 normalAt(vec3 pos)
{
	...
	return normalize(mix(
	    mix(mix(sdfGrad(i),              sdfGrad(i+ivec3(1,0,0)), f.x),
	        mix(sdfGrad(i+ivec3(0,1,0)), sdfGrad(i+ivec3(1,1,0)), f.x), f.y),
	    mix(mix(sdfGrad(i+ivec3(0,0,1)), sdfGrad(i+ivec3(1,0,1)), f.x),
	        mix(sdfGrad(i+ivec3(0,1,1)), sdfGrad(i+ivec3(1,1,1)), f.x), f.y),
	    f.z));
}

// ---- Marching Cubes (identical to geometry_generation_new.comp.glsl) ----
...
void main()
{
	...
	// Zero all 15 slots for this cell so unused triangle slots are degenerate
	for (int z = 0; z < 15; z++)
		storePosition(idx * 15 + z, vec3(0.0));
	...
	for (i = 0; i < 5; i++)
	{
		...
		if (_TriangleConnectionTable[flagIndex * 16 + 3 * i] >= 0)
		{
			...
			vec2 uv0 = mix(uvX, uvZ, abs(v0.normal.z));
			uv0 = mix(uv0, uvY, abs(v0.normal.y));
